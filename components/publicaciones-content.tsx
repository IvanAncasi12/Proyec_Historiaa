...
import { api, Publicacion } from '@/lib/api';

interface PublicacionesContentProps {
  colors: {
    primario: string;
    secundario: string;
    terciario: string;
...
export default function PublicacionesContent({ colors }: PublicacionesContentProps) {
  ...
  useEffect(() => {
    const cargarTodo = async () => {
      try {
        ...
        setPublicaciones(resourcesData.upea_publicaciones || []);
    ...
    return (
      <section className="py-24 bg-white min-h-screen flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-4 px-7 py-5 rounded-3xl border border-white/80 bg-white/75 backdrop-blur-xl shadow-2xl shadow-slate-300/40">
          ...
          <div>
            <p className="text-slate-900 text-lg font-black">Cargando publicaciones</p>
            <p className="text-slate-500 text-sm">Preparando artículos y documentos...</p>
          </div>
        </div>
      </section>
    );
            ...
            {publicaciones.length === 0 ? (
              <div className="relative overflow-hidden rounded-[2rem] border bg-white/78 backdrop-blur-xl p-16 text-center shadow-2xl shadow-slate-300/30">
                ...
                <p className="text-slate-500">
                  Pronto compartiremos nuevos artículos.
                </p>
              </div>
            ) : (
                ...
                {publicaciones.map((pub, index) => (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col gap-5">
                      ...
                      <div
                        className="w-full h-56 rounded-[1.5rem] overflow-hidden cursor-pointer group/image border bg-white shadow-xl"
                        style={{
                          borderColor: `${colors.primario}22`,
                          boxShadow: `0 18px 40px rgba(15,23,42,0.12)`,
                        }}
                        onClick={() => pub.publicaciones_imagen && setSelectedImage(pub.publicaciones_imagen)}
                      >
                        {pub.publicaciones_imagen ? (
                          <img
                            src={pub.publicaciones_imagen}
                      ...
                      <div>
                        ...
                        >
                          {pub.publicaciones_tipo || 'Artículo'}
                        </span>
                        ...
                        <div className="flex flex-col gap-3 text-sm text-slate-600">
                          {pub.publicaciones_fecha && (
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                <b className="text-slate-800">Fecha:</b> {formatDate(pub.publicaciones_fecha)}
                              </span>
                            </div>
                          )}
                          ...
                          {pub.publicaciones_autor && (
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                <b className="text-slate-800">Autor:</b> {pub.publicaciones_autor}
                              </span>
                            </div>
                          )}
                        </div>
                        ...
                        {pub.publicaciones_documento && (
                          <a
                            href={pub.publicaciones_documento}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-black text-white transition-all hover:scale-105"
                            style={{
                              background: `linear-gradient(135deg, ${colors.primario}, ${colors.terciario})`,
            ...
            <div className="sticky top-24 space-y-6">
              ...
              >
                ...
                <p className="text-sm text-slate-500 mb-3">
                  Consulta publicaciones, artículos y documentos disponibles.
                </p>
