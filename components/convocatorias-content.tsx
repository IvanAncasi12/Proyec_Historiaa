...
import { 
  ...
  TrendingUp, Award, Briefcase, X
} from 'lucide-react';
import { api, Convocatoria } from '@/lib/api';

interface ConvocatoriasContentProps {
  colors: { primario: string; secundario: string; terciario: string; };
}

export default function ConvocatoriasContent({ colors }: ConvocatoriasContentProps) {
  ...
  useEffect(() => {
    const cargarTodo = async () => {
      try {
        ...
        const soloConvocatorias = todos.filter(c => {
          const tipo = c.tipo_conv_comun?.tipo_conv_comun_titulo?.toLowerCase() || '';
          const titulo = c.con_titulo?.toLowerCase() || '';
          return tipo.includes('convocatoria') || titulo.includes('convocatoria');
        });
        
        setConvocatorias(soloConvocatorias);
        
        setCounts({
          convocatorias: soloConvocatorias.length,
    ...
    return (
      <section className="py-24 bg-white min-h-screen flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-4 px-7 py-5 rounded-3xl border border-white/80 bg-white/75 backdrop-blur-xl shadow-2xl shadow-slate-300/40">
          ...
          <div>
            <p className="text-slate-900 text-lg font-black">Cargando convocatorias</p>
            <p className="text-slate-500 text-sm">Preparando oportunidades disponibles...</p>
          </div>
        </div>
      </section>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
              ...
              <div className="grid grid-cols-1 gap-7">
                {convocatorias.map((convocatoria, index) => (
                  <div 
                    ...
                    className="group relative overflow-hidden rounded-[2rem] border bg-white/78 backdrop-blur-xl p-5 transition-all duration-500 hover:-translate-y-2"
                    style={{
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-6">
                      ...
                      <div 
                        ...
                        onClick={() => convocatoria.con_foto_portada && setSelectedImage(convocatoria.con_foto_portada)}
                      >
                        {convocatoria.con_foto_portada ? (
                          <img 
                            src={convocatoria.con_foto_portada} 
                        ...
                        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 text-sm text-slate-600">
                          {convocatoria.con_fecha_inicio && (
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                <b className="text-slate-800">Inicio:</b> {formatDate(convocatoria.con_fecha_inicio)}
                              </span>
                            </div>
                          )}
                          ...
                          {convocatoria.con_fecha_fin && (
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                <b className="text-slate-800">Fin:</b> {formatDate(convocatoria.con_fecha_fin)}
                              </span>
                            </div>
                          )}
                        </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              ...
              >
                <FileText className="w-10 h-10 mx-auto mb-3" style={{ color: colors.primario }} />
                <a
                  href="/contacto"
