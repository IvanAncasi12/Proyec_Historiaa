...
export default function AvisosContent({ colors }: AvisosContentProps) {
  const [avisos, setAvisos] = useState<Convocatoria[]>([]);
    ...
    const cargarTodo = async () => {
      try {
        ...
        setCounts({
          ...
          comunicados: todos.filter(c => {
            const tipo = c.tipo_conv_comun?.tipo_conv_comun_titulo?.toLowerCase() || '';
            return tipo.includes('comunicado');
          }).length,
          avisos: soloAvisos.length,
  ...
  const categoriesData = [
    ...
    { name: 'Avisos', count: counts.avisos, href: '/avisos', icon: AlertCircle },
    ...
    return (
      <section className="py-24 bg-white min-h-screen flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-4 px-7 py-5 rounded-3xl border border-white/80 bg-white/75 backdrop-blur-xl shadow-2xl shadow-slate-300/40">
          ...
          <div>
            <p className="text-slate-900 text-lg font-black">Cargando avisos</p>
            <p className="text-slate-500 text-sm">Preparando avisos importantes...</p>
          </div>
        </div>
      </section>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
            ...
            {avisos.length === 0 ? (
              <div className="relative overflow-hidden rounded-[2rem] border bg-white/78 backdrop-blur-xl p-16 text-center shadow-2xl shadow-slate-300/30">
                ...
                <p className="text-slate-500">
                  Pronto publicaremos nuevos avisos.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-7">
                {avisos.map((aviso, index) => (
                  <div 
                    key={aviso.idconvocatorias}
                    className="group relative overflow-hidden rounded-[2rem] border-l-4 bg-white/78 backdrop-blur-xl p-5 transition-all duration-500 hover:-translate-y-2"
                    style={{
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-6">
                      ...
                      <div 
                        className="w-full md:w-72 h-56 rounded-[1.5rem] overflow-hidden flex-shrink-0 cursor-pointer group/image border bg-white shadow-xl"
                        style={{
                          borderColor: `${colors.terciario}22`,
                          boxShadow: `0 18px 40px rgba(15,23,42,0.12)`,
                        }}
                        onClick={() => aviso.con_foto_portada && setSelectedImage(aviso.con_foto_portada)}
                      >
                        {aviso.con_foto_portada ? (
                          <img 
                            src={aviso.con_foto_portada} 
                      ...
                      <div className="flex-1">
                        <div className="mb-4">
                          <span 
                            className="inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-black text-white mb-3 shadow-lg"
                            style={{
                        ...
                        <p className="text-slate-600 text-sm mb-5 line-clamp-2 leading-relaxed">
                          {aviso.con_descripcion}
                        </p>

                        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 text-sm text-slate-600">
                          {aviso.con_fecha_inicio && (
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                <b className="text-slate-800">Fecha:</b> {formatDate(aviso.con_fecha_inicio)}
                              </span>
                            </div>
                          )}
                        </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              ...
              >
                <AlertCircle className="w-10 h-10 mx-auto mb-3" style={{ color: colors.terciario }} />

                <p className="text-sm text-slate-500 mb-3">
                  Revisa avisos importantes y anuncios recientes de la carrera.
                </p>
